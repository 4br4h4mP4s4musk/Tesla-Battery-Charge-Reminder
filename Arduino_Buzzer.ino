...
// this function to their phone app in the future but in the mean time, I've built a simple circuit based on an Arduino Nano.  
...
// weight of the cables. The switch is monitored by the Nano. 
// The Nano controls an HC-SR04 ultrasonic sensor which is aimed at the Tesla that is about 3 feet away when in the garage. 
// If the Tesla is not in the garage, the HC-SR04 measures over 10 feet to the far wall. An LED is controlled by the Nano 
// that blinks 1 to 9 times to indicate a distance of 1 to 9 feet. Blink rate is 1 second. If the distance is 10 feet or more, 
// the LED blinks slowly at a rate of 5 seconds "on" & 5 seconds "off". The Nano repeats the loop every 10 seconds. 
// The circuit is powered by a 5 Volt 1 Amp wall wart supply.     
//
// Release History
// Dec 31, 2021  Original Release

// Nano Pins 
#define Trig 3 // Logic output to the Trig pin of the HC-SR04. (10us high pulse to trigger)
#define Echo 4 // Logic input from the Echo pin of the HC-SR04 (High pulse duration is round trip time)
#define Switch 0 // Logic input from the pulled up push button switch in the charge cable wall hook (1=Open)
#define Buzzer 1 // Logic output to turn on the active buzzer (thru an NPN transistor). (1=On)
#define LED 2 // Logic output to turn on an LED (1=On)
// Globals
